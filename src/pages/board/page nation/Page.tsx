...
type PageProps = {
    totalPosts?: number; // ✅ 올바르게 수정
  }

const Page:React.FC<PageProps> = ({ totalPosts }) => {
    const [currentPageSet, setCurrentPageSet] = useState<number>(0);
    const postsPerPage:number = 3;
    const totalPages:number = totalPosts ? Math.ceil((totalPosts / postsPerPage)-1) : 0; // 전체 페이지수 :totalPages
    const pagesPerSet:number = 5; // 0~ 몇 개 까지를 최대로 보여줄지 정함 : pagesPerSet
    const startPage:number = currentPageSet * pagesPerSet;
    const endPage:number = Math.min(startPage + pagesPerSet - 1, totalPages);
